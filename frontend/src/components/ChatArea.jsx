      ...
      <div className="chat-area">
        <div className="empty-state">
          ...
          <div className="suggestion-chips">
            <button className="suggestion-chip" onClick={() => { const el = document.getElementById('queryInput'); if (el) el.value = '调研2025年国产AI芯片市场并生成分析报告' }}>国产芯片市场调研</button>
            <button className="suggestion-chip" onClick={() => { const el = document.getElementById('queryInput'); if (el) el.value = '对比分析 React 和 Vue 在2025年的生态和发展趋势' }}>React vs Vue</button>
            <button className="suggestion-chip" onClick={() => { const el = document.getElementById('queryInput'); if (el) el.value = '写一篇关于大语言模型发展的技术报告' }}>LLM报告</button>
          </div>
        </div>
      </div>
    )
      ...
      {conv.messages.map((m, i) => (
        <div key={i} className={'message fade-up ' + (m.role === 'user' ? 'user' : 'assistant')} style={m.role==='user'?{position:'relative'}:{}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            {m.dag && <div className="dag-container"><DAGView data={m.dag} /></div>}
            {m.dag && <AgentStepper />}
            {m.role === 'user' && (
              <div className="bubble-actions">
                <button className="bubble-action-btn" onClick={() => { const el = document.getElementById('queryInput'); if (el) { el.value = m.content; el.focus() } }}>✎ Edit</button>
              </div>
            )}
          </div>
        </div>
  ...
  useEffect(() => {
    const render = () => {
      try {
        ...
        if (window.mermaid) {
          window.mermaid.render('dag-' + Date.now(), mm).then(r => setSvg(r.svg))
        }
      } catch(e) {}
    }
    if (!window.mermaid) {
