...
async function q(path: string, input: Record<string, unknown>) {
  const r = await fetch(`/api/trpc/${path}?input=${encodeURIComponent(JSON.stringify(input))}`);
  if (!r.ok) throw new Error(await r.text());
  return (await r.json()).result.data.json;
}
async function m(path: string, input: Record<string, unknown>) {
  const r = await fetch(`/api/trpc/${path}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ json: input }) });
  if (!r.ok) throw new Error(await r.text());
  return (await r.json()).result.data.json;
}
...
export default function Home() {
  ...
  const cs = useCallback((id: string) => {
    ...
    const e = new EventSource(`/api/task/${id}/stream`); er.current = e;
    e.onmessage = (ev) => {
      try {
  ...
  const hs = async (q: string) => {
    ...
    try { const r = await m("task.submit",{query:q,convId:cid,lang:localStorage.getItem("lang")||"en"}); setTid(r.taskId); cs(r.taskId); }
