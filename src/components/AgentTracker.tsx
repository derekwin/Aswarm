...
type Agent = { name: string; role: string; state: string; subtaskId: string; output?: string; error?: string; retries?: number };

function SkeletonRow() {
  return <div className="flex items-center gap-2 w-full py-1.5 px-3 animate-shimmer"><span className="w-1.5 h-1.5 rounded-full shrink-0 bg-zinc-700" /><span className="h-3 rounded bg-zinc-700 flex-1" /><span className="h-3 w-12 rounded bg-zinc-700 shrink-0" /></div>;
}

export function AgentTracker({ agents, execState, onAgentClick, onFilesClick }: {
  agents: Record<string, Agent>; execState: string;
  onAgentClick?: (agent: Agent) => void; onFilesClick?: () => void;
}) {
  ...
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-800/50 overflow-hidden animate-fade-up">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-zinc-700/50">
        <span className={`w-1.5 h-1.5 rounded-full ${anyRunning ? "bg-accent animate-pulse" : completed === list.length ? "bg-green-400" : "bg-zinc-500"}`} />
        ...
        {hasAgents && onFilesClick && <button onClick={onFilesClick} className="text-xs text-zinc-500 hover:text-zinc-300 px-1">📂</button>}
      </div>
        ...
        <div className={list.length > 1 ? "divide-y divide-zinc-700/50" : ""}>
          {list.map(a => (
            <button key={a.subtaskId} onClick={() => onAgentClick?.(a)} className="flex items-center gap-2 w-full text-left py-1.5 px-3 text-xs hover:bg-white/4 transition-colors">
              ...
              {(a.retries ?? 0) > 0 && <span className="text-warning text-[10px] shrink-0">↻{a.retries}</span>}
